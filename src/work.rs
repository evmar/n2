...
impl<'a> Work<'a> {
    ...
    /// Return whether an input file to a ready build step exists.
    /// Assumes the input dependencies have already executed, but otherwise
    /// may stat the file on disk.
    fn input_exists(file_state: &mut FileState, id: FileId, file: &File) -> anyhow::Result<bool> {
        let mtime = match file_state.get(id) {
            ...
            None => {
                ...
                file_state.restat(id, file.path())?
            }
        };
        return Ok(mtime != MTime::Missing);
    }
    ...
    fn ensure_discovered_stats(&mut self, id: BuildId) -> anyhow::Result<Option<FileId>> {
        ...
        for &id in build.discovered_ins() {
            if !Self::input_exists(&mut self.file_state, id, self.graph.file(id))? {
                return Ok(Some(id));
            }
        }
